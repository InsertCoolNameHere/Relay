import logging
import argparse
from cluster.detect_cluster import cluster_setup_nlcd
import os
...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, training_type = 1, my_type ="ANCHOR", logger=None):
    ...
    if transferLearning:
        # FETCH THE OLD MODEL
        base_model, parent_err = trained_models[parent_quad]["model_obj"], trained_models[parent_quad]["model_err"]
    else:
        base_model = from_scratch(training_type)

    fancy_logging("MODEL INIT FOR GISJOIN %s TIME TAKEN: %s" % (quad_to_train, str(time()-start_time)), my_hostname, logger)

    # CHANGE THIS TO LOCAL CODE
    if training_type == 1:
        trained_model, val_loss, epochs = modeling_ae(quad_to_train, parent_err, transferLearning, base_model, my_hostname,
                                              epochs=num_epochs, sample_percent=sample_percent, logger=logger)
    else:
        trained_model, val_loss, epochs = modeling_ip(quad_to_train, parent_err, transferLearning, base_model, my_hostname,
                                              epochs=num_epochs, sample_percent=sample_percent, logger=logger)

    start_tim1 = time()

    model_infos = {"model_obj": trained_model, "model_err": val_loss}
    if not transferLearning:
        # x=1
        trained_models[quad_to_train] = model_infos
    ...
    fancy_logging('FINISHED MODELLING FOR %s C/O %s TYPE: %s TOTAL_TIME_TAKEN %s VAL_LOSS: %s PARENT_LOSS %s NUM_EPOCHS: %d'
                  % (quad_to_train,my_type, parent_quad, str(time()-start_time), str(val_loss), str(parent_err), epochs), my_hostname, logger)
...
if __name__ == '__main__':
    ...
    log_file_name = base_path + 'tl_nlcd_' + my_hostname + "_" + str(ae_or_clas) + '.log'
    ...
    centroid_map = cluster_setup_nlcd(img_dir, "mapz/cluster_centroids", "mapz/cluster_lite")
    i=0
    ...
    for quad_key in centroid_map:
        quad_to_train = quad_key
        execute_ml(quad_to_train, 'default', my_hostname, sample_percent, num_epochs, ae_or_clas, my_type, logger)
        i+=1

    fancy_logging("TRAINING ALL PARENTS DONE.............." + str(time() - start_time), my_hostname, logger)
    # **********************TRAIN THE DEPENDENTS

    fancy_logging("TRAINING CHILDREN..............", my_hostname, logger)
    start_time = time()
    my_type = "DEPENDENT"
    i = 0
    ...
    for quad_key in centroid_map:
        for quad_to_train in centroid_map[quad_key]:
            # THE CENTROID FOR THIS PARTICULAR QUADHASH
            parent_quad = quad_key
