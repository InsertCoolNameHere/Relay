...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, mode = 1, logger=None):
    ...
    if transferLearning:
        # FETCH THE OLD MODEL
        base_model, parent_err = fetch_model_MONGO_MEMORY(parent_quad, trained_models)
        # IF NOTHING CAME BACK
        if not base_model:
            print("THIS GISJOIN IS AN ORPHAN :(")
            fancy_logging("THIS GISJOIN %s IS AN ORPHAN :( .... TRAINING FROM SCRATCH"%(quad_to_train), my_hostname, logger)
            base_model = from_scratch(mode)
            transferLearning = False
    else:
        base_model = from_scratch(mode)

    # CHANGE THIS TO LOCAL CODE
    trained_model, val_loss = modeling_ae(quad_to_train, parent_err, transferLearning, base_model, my_hostname, epochs=num_epochs, sample_percent=sample_percent, logger=logger)

    # SAVING TRAINED MODEL IN-MEMORY AND IN MONGO-DB
    save_model(trained_model, quad_to_train, trained_models, val_loss, isTL=transferLearning)
    fancy_logging('FINISHED MODELLING FOR %s TOTAL_TIME_TAKEN %s' % (quad_to_train, str(time()-start_time)), my_hostname, logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nep', '--epochs', help='Port for The Coordinator Machine', required=False, type=int, default=2)
    ...
    log_file_name = base_path + 'tl_' + my_hostname + "_"+ str(mode)+ '.log'
    ...
    for quad_dict in my_centroids:
        ...
        execute_ml(quad_to_train, 'default', my_hostname, sample_percent, num_epochs, mode, logger)
        i+=1
    ...
    for quad_to_train in remainder_quads:
        ...
        execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, mode, logger)
        i += 1