...
class RedisWQ(object):
    ...
    This work my_que is finite: as long as no more work is added
    after workers start, the workers can detect when the my_que
    is completely empty.

    The items in the work my_que are assumed to have unique values.
    ...
    def __init__(self, name, **redis_kwargs):
        ...
        The work my_que is identified by "name".  The library may create other
        keys with "name" as a prefix.
        """
        self._db = redis.StrictRedis(**redis_kwargs)
        # The session ID will uniquely identify this "worker".
        self._session = str(uuid.uuid4())
        # Work my_que is implemented as two queues: main, and processing.
    ...
    def _main_qsize(self):
        """Return the size of the main my_que."""
    ...
    def empty(self):
        """Return True if the my_que is empty, including work being done, False otherwise.
    ...
    #        """Return to the work queueReturn True if the my_que is empty, False otherwise."""
    ...
    #          # then move the item back to the main my_que so others can work on it.
    #          if not self._lease_exists(item):
    #            TODO: transactionally move the key from processing my_que to
    #            to main my_que, while detecting if a new lease is created
    #            or if either my_que is modified.
    ...
    def lease(self, lease_secs=60, block=True, timeout=None):
        """Begin working on an item the work my_que.
        ...
        if item:
            ...
            # for this item a later return it to the main my_que.
...
# TODO: add a function to add an item to the my_que.  Atomically
# check if the my_que is empty and if so fail to add the item
