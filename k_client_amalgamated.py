...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, training_type = 1, my_type ="ANCHOR", logger=None):
    ...
    if not base_model:
        print("ERROR!!!: COULD NOT FETCH PARENT FROM MONGODB/MEMORY")
        exit(1)

    # CHANGE THIS TO LOCAL CODE
    if training_type == 1:
        trained_model, val_loss, epochs = modeling_ae(quad_to_train, parent_err, transferLearning, base_model, my_hostname, epochs=num_epochs, sample_percent=sample_percent, logger=logger)
    else:
        trained_model, val_loss, epochs = modeling_ip(quad_to_train, parent_err, transferLearning, base_model, my_hostname, epochs=num_epochs, sample_percent=sample_percent, logger=logger)
    ...
    fancy_logging('FINISHED MODELLING FOR %s TYPE: %s TOTAL_TIME_TAKEN %s VAL_LOSS: %s PARENT_LOSS %s NUM_EPOCHS %d'
                  % (quad_to_train,my_type, str(time()-start_time), str(val_loss), str(parent_err), epochs), my_hostname, logger)
...
if __name__ == '__main__':
    ...
    log_file_name = base_path + 'tl_amalgamated_' + my_hostname + "_" + str(ae_or_clas) + '.log'
    ...
    fancy_logging("TRAINING PARENTS...BUT AMALGAMATED..............", my_hostname, logger)
    start_time = time()
    ...
    fancy_logging("TRAINING CHILDREN...BUT AMALGAMATED..............", my_hostname, logger)
    start_time = time()
    my_type = "ANCHOR"
