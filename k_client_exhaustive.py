...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, training_type = 1, my_type ="ANCHOR", logger=None):
    ...
    fancy_logging('FINISHED MODELLING FOR %s TYPE: %s TOTAL_TIME_TAKEN %s VAL_LOSS: %s NUM_EPOCHS %d'
                  % (quad_to_train,my_type, str(time()-start_time), str(val_loss), epochs), my_hostname, logger)
...
if __name__ == '__main__':
    ...
    log_file_name = base_path + 'tl_dumb_' + my_hostname + "_" + str(ae_or_clas) + '.log'
