...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, training_type = 1, my_type ="ANCHOR", logger=None):
    ...
    if training_type == 1:
        trained_model, val_loss = modeling_ae(quad_to_train, parent_err, transferLearning, base_model, my_hostname,
                                              epochs=num_epochs, sample_percent=sample_percent, logger=logger)
    else:
        trained_model, val_loss = modeling_ip(quad_to_train, parent_err, transferLearning, base_model, my_hostname,
                                              epochs=num_epochs, sample_percent=sample_percent, logger=logger)
    ...
    fancy_logging('FINISHED MODELLING FOR %s C/O %s TYPE: %s TOTAL_TIME_TAKEN %s VAL_LOSS: %s' % (quad_to_train,my_type, parent_quad, str(time()-start_time), str(val_loss)), my_hostname, logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nep', '--epochs', help='Port for The Coordinator Machine', required=False, type=int, default=1)
    ...
    log_file_name = base_path + 'tl_' + my_hostname + "_" + str(ae_or_clas) + '.log'
    ...
    for quad_to_train in remainder_quads:
        ...
        if quad_to_train in parent_quad:
            print(">>>>>>>>>>>>>>>NO NEED TO TRAIN THE PARENT AGAIN<<<<<<<<<<<<<<<<<<<")
            continue
