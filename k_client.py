...
def execute_ml(quad_to_train, parent_quad, my_hostname, sample_percent, num_epochs, training_type = 1, my_type ="ANCHOR", logger=None):
    ...
    if transferLearning:
        ...
        while not base_model:
            print("THIS GISJOIN'S PARENT HAS NOT COMPLETED TRAINING ON SOME OTHER NODE... TRYING AGAIN AFTER 5 SECS")
            fancy_logging("THIS GISJOIN %s's PARENT HAS NOT COMPLETED TRAINING ON SOME OTHER NODE... TRYING AGAIN AFTER 5 SECS"%(quad_to_train), my_hostname, logger)
            sleep(10)
            start_time = time()
            base_model, parent_err = fetch_model_MONGO_MEMORY(parent_quad, trained_models, training_type)
